#!/usr/bin/env python3
"""Benchmark Terra models with speed_bench.py using their real llama-swap args."""
...
def resolve_server_args(model_id: str) -> list[str]:
    """Substitute the real bench port into a catalog model's llama-server args."""
    return [
        str(SERVER_PORT) if part == "${PORT}" else part
        for part in CATALOG_BY_ID[model_id]["args"]
    ]


MODELS = [
    (model_id, Path(MODEL_DIR) / CATALOG_BY_ID[model_id]["files"][0], resolve_server_args(model_id))
...
def config_fingerprint(
    ...
    """Identify a (model file, server config, bench config) triple so repeats can be skipped.

    Keyed on the model file's size+mtime rather than just its path, so a
    re-quantized or replaced .gguf under the same filename is treated as untested,
    and on the full server_args so a catalog config change (e.g. a new
    --spec-draft-p-min) is treated as untested too.
    """
...
def run_benchmark(
    ...
    """Start one model through llama-server with its real args and run speed_bench.py."""
    if not model_path.is_file():
        raise FileNotFoundError(model_path)

    command = [LLAMA_CPP_SERVER, *server_args]
    result_path = output_dir / f"{model_id}.json"
    log_path = output_dir / f"{model_id}.server.log"
    print(f"[{model_id}] {' '.join(command)}", flush=True)
    ...
    print(
        f"[{model_id}] decode={overall['avg_pred_t_s']:.2f} tok/s "
...
def start_detached_worker(args: argparse.Namespace) -> int:
    ...
    unit = f"hermes-model-benchmark-{int(time.time())}"
    command = [
        "systemd-run",
        "--user",
        f"--unit={unit}",
...
def run_worker(args: argparse.Namespace) -> int:
    """Stop Hermes, isolate benchmarks, then restore the backend and Hermes."""
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    output_dir = DEFAULT_ARTIFACT_ROOT / timestamp
    output_dir.mkdir(parents=True, exist_ok=False)
    summary: dict[str, Any] = {
        "timestamp": timestamp,
        "speed_bench": str(args.speed_bench),
        "models": [],
    }
    gateway_was_active = (
        subprocess.run(
    ...
    pending: list[tuple[str, Path, list[str], str]] = []
    for model_id, model_path, server_args in MODELS:
        if not model_path.is_file():
            print(f"[{model_id}] skip: model file missing at {model_path}", flush=True)
            continue
        ...
        if fingerprint in already_tested:
            print(f"[{model_id}] skip: already benchmarked (fingerprint {fingerprint})", flush=True)
            continue
        pending.append((model_id, model_path, server_args, fingerprint))
    ...
    try:
        if pending:
            ...
            unload_backend()
            for model_id, model_path, server_args, fingerprint in pending:
            ...
            print(
                "Nothing to benchmark — every model/config is already in results.jsonl",
                flush=True,
            )
